...
def correlation_heatmap(data,
    ...
    rotate_xlabels(ax)
    rotate_ylabels(ax)
    ax.set(title = f"{method.title() }'s Correlation Matrix Heatmap")
...
def cramers_v(data, var1, var2):
    ...
    # Ensure phi2 is a scalar
    if isinstance(phi2, np.ndarray):
        phi2 = phi2.item()
...
def cramerV_heatmap(data, figsize = (12, 6), cmap = "Blues"):
    ...
    ax.set(title = "Craver's V Correlation Matrix Heatmap")
    rotate_xlabels(ax)
    rotate_ylabels(ax)
...
def cat_summary(data, var):
    ...
    # quick glance
    display_html(6, "Quick Glance")
    display(col)

    # meta data 
    display_html(5, "Meta Data")
    ...
    display_html(5, "Categories Distribution: ")
...
def cat_univar_plots(data, 
    ...
    display_html(5, f"Univariate Analysis of {var}")
    display_html(content="")

    fig = plt.figure(figsize = figsize)
    gs = GridSpec(2, 2, figure = fig)
    ax1 = fig.add_subplot(gs[0, 0]) # bar plot 
...
def num_num_hyp_testing(data, var1, var2, alpha=0.05):
    ...
    # pearson test
    pearson_statistic, pearson_pvalue = stats.pearsonr(temp[var1].values, temp[var2].values)
    ...
    print(f"- {'Test Statistic':21}: {pearson_statistic}")
    print(f"- {'p-value':21}: {pearson_pvalue}")
    ...
    # spearman test
    spearman_statistic, spearman_pvalue = stats.spearmanr(temp[var1].values, temp[var2].values)
    display_html(3, "Spearman Test")
    print(f"- {'Significance Level':21}: {alpha * 100}%")
    print(f"- {'Null Hypothesis':21}: The samples are uncorrelated")
    print(f"- {'Alternate Hypothesis':21}: The samples are correlated")
    print(f"- {'Test Statistic':21}: {spearman_statistic}")
    print(f"- {'p-value':21}: {spearman_pvalue}")
